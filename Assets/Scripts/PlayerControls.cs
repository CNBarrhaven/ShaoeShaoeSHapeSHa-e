...
public class PlayerControls : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((Input.GetKey("space") || Input.GetKey("z")) && (Time.time > nextFire)) 
        {
            ...
            if (currentLevel >= 5)
            {
                Vector3 rightsOffSet = new Vector3(-0.4f, 0, 0);
                Vector3 leftsOffSet = new Vector3(0.4f, 0, 0);

                Instantiate(projectile, transform.position + rightsOffSet, transform.rotation);
                Instantiate(projectile, transform.position + leftsOffSet, transform.rotation);
            }
        }
